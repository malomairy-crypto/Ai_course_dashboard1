    ...
    <div className="flex flex-col gap-6 p-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 rounded-lg border border-[#e8e4dc] bg-white px-3 py-1.5 shadow-sm">
          ...
          <span className="text-[12px] text-[#5a5a5a]">Apr 22, 2026</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 xl:grid-cols-4">
        {metricCards.map((card, i) => (
          <div
            ...
            style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 2px 12px rgba(0,0,0,0.06)', animationDelay: `${0.06 + i * 0.07}s` }}
          >
            ...
            <div>
              ...
              <div className="mt-2 flex items-center gap-1.5 text-[12px]">
                ...
                {card.up === null && (
                  <span className="font-semibold text-[#f59e0b]">{card.change}</span>
                )}
                <span className="text-[#9a9a9a]">{card.period}</span>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        >
          <div className="mb-5 flex items-center justify-between">
            <div>
              ...
              <p className="mt-0.5 text-sm font-medium text-[#0f0e0e]">12 months — Mar 2025 to Feb 2026</p>
            </div>
        ...
        >
          <div className="mb-4">
            <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[#9a9a9a]">Activity</p>
            <p className="mt-0.5 text-sm font-medium text-[#0f0e0e]">Live feed</p>
          </div>
          <div className="flex flex-col gap-4">
            {recentActivity.map((item, i) => (
              <div key={i} className="flex items-start gap-3">
                <div className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-[#0f0e0e] ring-2 ring-[#f0eeea]" />
                <div className="flex min-w-0 flex-col gap-0.5">
                  <span className="text-[12px] font-medium leading-snug text-[#0f0e0e]">{item.action}</span>
                  <span className="truncate text-[11px] leading-snug text-[#9a9a9a]">{item.detail}</span>
                  <span className="text-[10px] text-[#c0bcb5]">{item.time}</span>
                </div>
              </div>
            ))}
          </div>
